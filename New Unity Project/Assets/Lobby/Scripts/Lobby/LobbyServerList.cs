    ...
    public class LobbyServerList : MonoBehaviour
    {
		...
		public void OnGUIMatchList(bool success, string extendedInfo, List<MatchInfoSnapshot> matches)
		{
			...
			for (int i = 0; i < matches.Count; ++i)
			{
				...
				if (matches [i].networkId.ToString () == GameStatesManager.Instance.RoomID) {
					AcceptInviteEnterLobbyy accept = GameObject.Find ("FBScriptHolder").GetComponent<AcceptInviteEnterLobbyy> ();
					accept.AcceptAndEnter (matches [i].networkId);
				}
            }
        }
